						...
						geocoder.getLatLng(
							...
							function(point) {
								if (!point) {
									...
									AUmap.addOverlay(createMarker(point, user));
								}
							}
						);
					}
...
function createMarker(point, user, img) {
    ...
    if(!img){
    var myHtml = '<div class="mapOverlay"><h3>' + user.location + '</h3>';
    myHtml += '<div>' + user.user;
    myHtml += ', Viewing : <a href="' + user.url + '">' + user.url + '</a></div></div>';
    }else{
